...
Page({
	...
	onLoad() {
		...
		const historyList: PoiPathHistoryInfoType[] = (wx.getStorageSync(STORAGE_KEY.TUAN_URL_HISTORY_LIST) || []).map(
		...
		const content = wx.getStorageSync(STORAGE_KEY.TUAN_URL_INPUT_CONTENT) || ''
	...
	handleInput(event: any) {
		...
		wx.setStorage({
			key: STORAGE_KEY.TUAN_URL_INPUT_CONTENT,
			data: content,
			success() {
	...
	bindCreatePoiPathTap() {
		...
		if (content) {
			...
			if (url) {
				...
				wx.setStorage({
					key: STORAGE_KEY.TUAN_URL_HISTORY_LIST,
