		...
		wx.getSetting({
			...
			success: function (res) {
				...
				if (!mainSwitch) {
					...
					const currentSubscriptionInfo = {}
					currentSubscriptionInfoList
						.filter(info =>
							that.subscriptionInfoList.find(
								item => item.tmplId === info.tmplId && item.type !== info.type,
							),
						)
						.forEach(({ tmplId, type }) => {
					...
					// 是否全部永久订阅
					const isAllPass = currentSubscriptionInfoList.every(({ type }) =>
						['accept', 'acceptWithForcePush'].includes(type),
					)
					// 是否保持以上选择
					console['log'](
						`===========设置页返回后查询订阅状态`,
						currentSubscriptionInfoList,
						that.subscriptionInfoList,
					)
